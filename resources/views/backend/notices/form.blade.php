...
@section('content')
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <div>
            <h1 class="h3 mb-1 text-gray-800">{{ isset($data) ? 'Edit Notice' : 'Create Notice' }}</h1>
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-0">
                    <li class="breadcrumb-item"><a href="{{ route('dashboard.index') }}">Dashboard</a></li>
                    <li class="breadcrumb-item"><a href="{{ route('notices.index') }}">Notices</a></li>
                    <li class="breadcrumb-item active" aria-current="page">
                        {{ isset($data) ? 'Edit Notice' : 'Create Notice' }}
                    </li>
                </ol>
            </nav>
        </div>

        <a href="{{ route('notices.index') }}" class="d-none d-sm-inline-block btn btn-sm btn-primary shadow-sm">
    ...
    <div class="card shadow mb-4 border-left-primary">
        <div class="card-header py-3 bg-white">
            <h6 class="m-0 font-weight-bold text-primary">
                <i class="fas fa-bullhorn mr-2"></i>{{ isset($data) ? 'Update Notice Information' : 'Notice Information' }}
            </h6>
        </div>

        <form action="{{ isset($data) ? route('notices.update', $data->id) : route('notices.store') }}" method="POST"
            ...
            <div class="card-body">
                <div class="row">
                        ...
                        <div class="card border-left-info shadow-sm h-100">
                            <div class="card-header bg-light">
                                <h6 class="m-0 font-weight-bold text-info">
                                    <i class="fas fa-file-upload mr-1"></i> Notice File
                                </h6>
                            </div>
                            <div class="card-body">
                                <div class="border rounded bg-light d-flex align-items-center justify-content-center mb-3"
                                    ...
                                    <div class="text-center text-muted px-3">
                                        ...
                                        <div class="font-weight-bold text-dark" id="noticeFileName">
                                            {{ isset($data) && $data->file ? basename($data->file) : 'Upload notice document' }}
                                        </div>
                                ...
                                <div class="custom-file">
                                    <input type="file" id="file" name="file"
                                        ...
                                        accept=".pdf,.doc,.docx,.xlsx,.xls,.ppt,.pptx" onchange="previewNoticeFile(event)">
                    ...
                    <div class="col-lg-8">
                        <ul class="nav nav-tabs nav-fill mb-4" id="noticeLanguageTabs" role="tablist">
                            <li class="nav-item">
                        ...
                        <div class="tab-content" id="noticeLanguageTabsContent">
                                    ...
                                    <div class="card-body">
                                        <div class="form-group mb-3">
                                            ...
                                            <input type="text" class="form-control @error('title_en') is-invalid @enderror"
                                                ...
                                                placeholder="Enter English notice title">
                                            @error('title_en')
                                    ...
                                    <div class="card-body">
                                        <div class="form-group mb-3">
                                            <label for="title_ne" class="form-label">
                                                <i class="fas fa-heading text-warning"></i> Notice Title
                                            </label>
                                            <input type="text" class="form-control @error('title_ne') is-invalid @enderror"
                                                ...
                                                placeholder="Enter Nepali notice title">
                                            @error('title_ne')
                                                <div class="invalid-feedback">{{ $message }}</div>
                                            @enderror
                                        </div>
                        ...
                        <div class="custom-control custom-switch custom-control-lg mt-4">
                            ...
                            <p class="small text-muted">Toggle to make this notice visible on the website.</p>
                        </div>

                        <div class="form-actions mt-4 border-top pt-4">
                            <button type="submit" class="btn btn-primary btn-icon-split shadow-sm">
                                <span class="icon text-white-50"><i class="fas fa-save"></i></span>
                                <span class="text">{{ isset($data) ? 'Update Notice' : 'Save Notice' }}</span>
                            </button>
                            <a href="{{ route('notices.index') }}" class="btn btn-secondary btn-icon-split shadow-sm ml-2">
...
@push('script')
    <script>
        function previewNoticeFile(event) {
            const fileName = event.target.value.split('\\').pop();
            event.target.nextElementSibling.innerText = fileName || 'Choose file...';
            document.getElementById('noticeFileName').innerText = fileName || 'Upload notice document';
        }
    </script>
@endpush
