...
@section('content')
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <div>
            <h1 class="h3 mb-1 text-gray-800">MVG Management</h1>
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-0">
                    <li class="breadcrumb-item"><a href="{{ route('dashboard.index') }}">Dashboard</a></li>
                    <li class="breadcrumb-item active" aria-current="page">MVG</li>
                </ol>
            </nav>
        </div>
        @can('MissionVision-Create')
            <a href="{{ route('mvg.create') }}" class="btn btn-primary btn-sm btn-icon-split shadow-sm">
                <span class="icon text-white-50">
                    <i class="fas fa-plus fa-sm"></i>
                </span>
                <span class="text">Create New MVG</span>
            </a>
        @endcan
    </div>
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-primary">
                <i class="fas fa-bullseye mr-2"></i>MVG List
            </h6>
        </div>
        <div class="card-body">
                        ...
                        @foreach ($lists as $list)
                            <tr>
                                ...
                                <td class="align-middle">
                                    ...
                                    <div class="small text-muted">{{ $list->subtitle_en ?? '-' }}</div>
                                </td>
                                <td class="align-middle">
                                    <div class="font-weight-bold text-dark">{{ $list->title_ne ?? '-' }}</div>
                                    <div class="small text-muted">{{ $list->subtitle_ne ?? '-' }}</div>
                                </td>
                                <td class="align-middle text-center">
                                ...
                                <td class="align-middle text-center">
                                    <div class="btn-group" role="group">
                                        @can('MissionVision-Edit')
                                            <a href="{{ route('mvg.edit', $list->id) }}"
                                               class="btn btn-info btn-sm shadow-sm"
                                               title="Edit">
                                                <i class="fas fa-pencil-alt"></i>
                                            </a>
                                        @endcan
                                        @can('MissionVision-Delete')
                                        ...
                                        <div class="modal-dialog modal-dialog-centered" role="document">
                                            <div class="modal-content border-0 shadow-lg">
                                                ...
                                                <div class="modal-body text-left p-4">
                                                    ...
                                                    <strong>"{{ $list->title_en ?? $list->title_ne ?? 'this MVG item' }}"</strong>?
                                                    <p class="text-muted small mt-2">This action cannot be undone.</p>
                                                </div>
                                                <div class="modal-footer bg-light">
                                                    <button type="button" class="btn btn-secondary btn-sm" data-dismiss="modal">Cancel</button>
                                                    <form method="POST" action="{{ route('mvg.destroy', $list->id) }}">
                                                        @csrf
                                                        @method('DELETE')
